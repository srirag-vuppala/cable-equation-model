...
class HodgkinHuxley():
    ...
    def derivatives_n(self, t,y, V):
        ...
        # dn/dt
        n = (self.alpha_n(v_temp) * (1.0 - y[0])) - (self.beta_n(v_temp) * y[0])
        self.counter += 1 
        print(self.counter)
    ...
    def derivatives_m(self, t,y, V):
        ...
        v_temp = V[self.counter] -(-70) 
        # dn/dt
        m = (self.alpha_m(v_temp) * (1.0 - y[1])) - (self.beta_m(v_temp) * y[1])
        self.counter += 1 
        self.counter = min(self.counter, cable.J - 1)
        return m 
    ...
    def derivatives_h(self, t,y, V):
        ...
        v_temp = V[self.counter] -(-70) 
        # dn/dt
        h = (self.alpha_h(v_temp) * (1.0 - y[2])) - (self.beta_h(v_temp) * y[2])
        self.counter += 1 
        self.counter = min(self.counter, cable.J - 1)
        return h 
    ...
    def main(self, t, V_old, timestep):
        ...
        # init_len = len(V_old)
        # for i in range(cable.J-init_len):
        #     V_old = np.append(V_old, 0)
        # #print(V_old)
            
        self.counter = 0 
        #Vy = odeint(self.derivatives, Y,t,  args=(V_old,), rtol = 1e9)
        Vyn = solve_ivp(self.derivatives_n, (0, timestep*cable.dt), Y,  args=(V_old,))
        self.counter = 0 
        Vym = solve_ivp(self.derivatives_m, (0, timestep*cable.dt), Y,  args=(V_old,))
        self.counter = 0 
        Vyh = solve_ivp(self.derivatives_h, (0, timestep*cable.dt), Y,  args=(V_old,))
        ...
        for i in range(cable.J):
            GK.append(self.g_K * np.power(n[i], 4.0))
            GNa.append(self.g_Na * np.power(m[i], 3.0) * h[i])
            GL.append(self.g_L) 
        ...
        #for i in range(cable.J):
            I.append((GK[i] * (V_old[i] - self.V_K)) + (GNa[i] * (V_old[i] - self.V_Na)) + (GL[i] * (V_old[i] - self.V_L)) - cable.Input_stimuli(cable.dt*i))
