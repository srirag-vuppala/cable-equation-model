...
def main():
    ...
    """ The time to integrate over  delta(t) = 0.1 ms;Creating one dimensional Time domain = 10 and N=1000 equally spaced grid points on the time domain"""
    T = 1000 
    N = 2000
    dt = float(T)/float(N)
    ...
    for i in range(1,N):
        ...
        #V_new = np.linalg.solve(A_v, B_v*V_grid of [i-1]in time  - I_ion(V_grid of [i-1] in time)
