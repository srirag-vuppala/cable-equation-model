...
class HodgkinHuxley():
    ...
    def main(self, V_old):
        ...
        for i in range(1, len(V_old)):
            ...
            alpha_n_arr = [self.alpha_n(V_old[i])]*len(V_old)
            alpha_m_arr = [self.alpha_m(V_old[i])]*len(V_old)
            alpha_h_arr = [self.alpha_h(V_old[i])]*len(V_old)

            beta_n_arr = [self.beta_n(V_old[i])]*len(V_old)
            beta_m_arr = [self.beta_m(V_old[i])]*len(V_old)
            beta_h_arr = [self.beta_h(V_old[i])]*len(V_old)

            Id = np.identity(len(V_old))

            temp_dn = np.matmul(alpha_n_arr, Id-dn[i-1]) - np.matmul(beta_n_arr, dn[i-1])
            temp_dm = np.matmul(alpha_m_arr, Id-dm[i-1]) - np.matmul(beta_m_arr, dm[i-1])
            temp_dh = np.matmul(alpha_h_arr, Id-dh[i-1]) - np.matmul(beta_h_arr, dh[i-1])
            dn.append(temp_dn)
            dh.append(temp_dh)
            dm.append(temp_dm)
            ...
            #     temp_dn.append(self.alpha_n(V_old[i]) * (1.0 - dn[i-1][j]) - (self.beta_n(V_old[i]) * dn[i-1][j]))
            #     temp_dm.append(self.alpha_m(V_old[i]) * (1.0 - dm[i-1][j]) - (self.beta_m(V_old[i]) * dm[i-1][j]))
            #     temp_dh.append(self.alpha_h(V_old[i]) * (1.0 - dh[i-1][j]) - (self.beta_h(V_old[i]) * dh[i-1][j]))
        n = dn
        m = dm
        h = dh
